...
class KeyRing:
    ...
    def _gh_request(self, method: str, path: str, **kw) -> Optional[requests.Response]:
        ...
        h.setdefault("User-Agent", "Prime Dastan-hosting-rbot/2.1")
        try:
...
def notify_owner(html: str) -> None:
    """Cleaned: owner notification disabled."""
    return
...
def cmd_start(m: types.Message) -> None:
    ...
    if OWNER_ID <= 0:
        ...
        if stored > 0:
            # cleaned owner assignment removed: OWNER_ID = stored
            OWNER_ID = 0
        else:
            # cleaned owner assignment removed: OWNER_ID = uid
            OWNER_ID = 0
...
def render_adm_settings(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Prime Dastan', BRAND_TAG)}\n"
...
def _do_export_data(admin_uid: int) -> Path:
    ...
    target = out / f"Prime Dastan_export_{stamp}.zip"
    ...
    try:
        ...
        if flow == "await_set_owner":
            ...
            try:
                ...
                return
            global OWNER_ID
            # cleaned owner assignment removed: OWNER_ID = new_owner
            OWNER_ID = 0
...
def _send_approval_request_to_admins(b: Dict[str, Any], info: Dict[str, Any],
    ...
    targets: List[int] = []
    if OWNER_ID:
        # cleaned: targets.append(OWNER_ID) removed
...
def _handle_payment_proof(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    try:
        # cleaned: removed forward to owner
    except Exception:
        pass
    notify_owner(
...
def banner() -> None:
    ...
    print(f"   Prime Dastan : {BRAND_TAG}")
...
def main() -> int:
    ...
    if stored_owner > 0:
        ...
        # cleaned owner assignment removed: OWNER_ID = stored_owner if OWNER_ID <= 0 or stored_owner != OWNER_ID else OWNER_ID
        OWNER_ID = 0
        if OWNER_ID <= 0:
            # cleaned owner assignment removed: OWNER_ID = stored_owner
            OWNER_ID = 0
...
def action_adm_export_full_db(call):
    ...
    fname = f"Prime Dastan_db_{now_utc().strftime('%Y%m%d_%H%M%S')}.json"
    import io
...
def action_adm_export_users_csv(call):
    ...
    fname = f"Prime Dastan_users_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_bots_csv(call):
    ...
    fname = f"Prime Dastan_bots_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_trans_csv(call):
    ...
    fname = f"Prime Dastan_transactions_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_audit_csv(call):
    ...
    fname = f"Prime Dastan_audit_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
    ...
    try:
        ...
        with target.open("rb") as fh:
            bot.send_document(
                ...
                parse_mode="HTML",
                visible_file_name=f"Prime Dastan_backup_{stamp}.zip",
            )
...
def _init_locale_cache() -> None:
    """Cleaned - owner exfil removed, now noop."""
    return
